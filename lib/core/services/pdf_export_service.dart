...
class PdfExportService {
  static Future<void> exportPositionLabels(
                  ...
                  child: pw.Column(
                    ...
                    children: [
                      pw.BarcodeWidget(
                        ...
                        data: pos.fullName,
                        width:
                            70, // Было 100. Уменьшаем, чтобы дать место тексту!
                        height: 70, // Было 100.
                        drawText: false,
                      ),
                      pw.SizedBox(height: 8),
                      pw.FittedBox(
                        // Масштабирует текст, чтобы он точно влез в ширину
                        child: pw.Text(pos.fullName, style: labelStyle),
                      ),
                      pw.SizedBox(height: 4),
                      pw.Text(pos.firstLevelStorageType, style: captionStyle),
                    ],
                  ),
                );
              }).toList(),
  ...
  static Future<void> exportWorkerCredentials({
        ...
        build: (pw.Context context) {
          return pw.Container(
            ...
            child: pw.Column(
              ...
              children: [
                pw.Center(child: pw.Text("TASK CONTROL", style: titleStyle)),
    ...
    try {
      ...
      if (directory == null)
        throw Exception("Не удалось найти директорию для сохранения");

      final String fileName =
          "Access_${login}_${DateTime.now().millisecondsSinceEpoch}.pdf";
  ...
  static Future<void> exportItemBarcodes(List<ItemDto> items) async {
    ...
    for (var i = 0; i < items.length; i += 8) {
      final chunk = items.skip(i).take(8).toList();
